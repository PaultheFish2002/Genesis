...
def main():
    ...
    num_drone = 5
    drones = [None] * num_drone
    for i in range(num_drone):
        drones[i] = scene.add_entity(
            morph=gs.morphs.Drone(
                file="urdf/drones/cf2x.urdf",
                pos=(0.2*(i-num_drone/2) + 0.0, 0.2*(i-num_drone/2)+ 0.0, 0.01*i +0.72),
            ),
        )
    ...
    if args.mac:
        gs.tools.run_in_another_thread(fn=run_sim, args=(scene, drones, args.vis))
        if args.vis:
            scene.viewer.start()
    else:
        run_sim(scene, drones, cam, args.vis)


def update_camera(cam, drone):
    ...
    cam.set_pose(pos=camera_pos, lookat=tuple(float(x) for x in drone_pos))

def run_sim(scene, drones, cam, enable_vis):
    traj = np.array(
        [
    ...
    for i in range(len(traj)*2):
        # 14468 is hover rpm
        for drone in drones:
            drone.set_propellels_rpm((1 + 0.05 * traj[i % len(traj)]) * 14468.429183500699)
        scene.step()
        # print(drones[2].get_pos())  # drone.get_pos()
        # cam.set_pose(
        # pos=(2.0 * np.sin(i / 60), 2.0 * np.cos(i / 60), 2.5),
        # lookat=(0, 0, 0.5),
        # )
        update_camera(cam, drones[2])
        time.sleep(1 / 60)
        cam.render()
    cam.stop_recording(save_to_filename="videoMD.mp4", fps=60)
