...
class HoverEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        self.controller_test = DronePIDController(drone=self.drone, dt=0.01 , base_rpm=self.base_rpm, pid_params=pid_params)
    ...
    def _at_target(self):
        at_target = (
            (torch.norm(self.rel_pos, dim=1) < self.env_cfg["at_target_threshold"]*2).nonzero(as_tuple=False).flatten()
        )
        return at_target
    ...
    def step(self, actions):
        ...
        # 测试
        # target_test = (self.commands[0]).cpu().tolist() 
        # prop_rpms = self.controller_test.update(target_test)
        
        # min_rpm = 0.9 * self.base_rpm
        # max_rpm = 1.5 * self.base_rpm      
          
        # prop_rpms = torch.clamp(prop_rpms, min_rpm, max_rpm)
        
        # 14468 is hover rpm
        # self.drone.set_propellels_rpm(prop_rpms.tile(self.num_envs,1))
        self.drone.set_propellels_rpm((1 + exec_actions * 0.8) * 14468.429183500699)
        # self.drone.set_propellels_rpm((1 + torch.zeros_like(exec_actions) * 0.8) * 14468.429183500699)
        # update target pos
